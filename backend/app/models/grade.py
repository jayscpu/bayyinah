...
class TeacherGrade(Base):
    ...
    final_grade: Mapped[int] = mapped_column(Integer, nullable=False)  # 0-5
    feedback: Mapped[Optional[str]] = mapped_column(Text)
    internal_notes: Mapped[Optional[str]] = mapped_column(Text)  # Teacher-only, not visible to student
    question_grades: Mapped[Optional[dict]] = mapped_column(JSON)  # {question_id: grade}
    total_points: Mapped[Optional[float]] = mapped_column(Float)  # sum of question grades
